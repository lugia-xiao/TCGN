...
import sys
sys.path.append("./other-CNNs")

class my_classifier(nn.Module):
    def __init__(self):
        super(my_classifier, self).__init__()
        self.classifier = nn.Sequential(nn.Linear(1024, 785, bias=True))

    def forward(self, x):
        return self.classifier(x)

class my_classifier_vgg(nn.Module):
    def __init__(self):
        super(my_classifier_vgg,self).__init__()
        self.classifier=nn.Sequential(nn.Linear(25088,25088*2,bias=True),
                                      nn.SELU(),nn.Dropout(0.2),
                                      nn.Linear(25088*2, 25088, bias=True),
                                      nn.SELU(), nn.Dropout(0.2),
                                      nn.Linear(25088, 4096, bias=True),
                                      nn.SELU(), nn.Dropout(0.2),
                                      nn.Linear(4096,785,bias=True)
        )
